...
export function RobotPrototype({
  ...
  useFrame((state, delta) => {
    ...
    const targetPosX = enableTranslation ? tx * maxMoveX : 0;
    ...
    const relativeX = enableTranslation
      ? (tx - bodyRef.current.position.x / 2.5)
      : tx;

    const maxHeadTurn = enableTranslation ? config.headLookY : 0.85;
    const maxBodyTurn = enableTranslation ? config.bodyTiltY : 0.45;

    const bodyTargetRotY = THREE.MathUtils.clamp(-relativeX * maxBodyTurn, -0.6, 0.6);
    const bodyTargetRotX = THREE.MathUtils.clamp(relativeX * relativeX * config.bodyTiltX - ty * 0.25, -0.35, 0.35);
    const bodyTargetRotZ = THREE.MathUtils.clamp(-relativeX * 0.15, -0.25, 0.25);
    ...
    const headTargetRotY = THREE.MathUtils.clamp(relativeX * maxHeadTurn, -0.75, 0.75);
    const headTargetRotX = THREE.MathUtils.clamp(-ty * config.headLookX, -0.4, 0.4);
